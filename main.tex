...
For a point force acting on the fluid the green function of the Stokes equation is called the Oseen tensor $\mm{T}^0$ and is given by
\begin{equation}
...
Introducing $\bm{R}_{ij} = |\bm{R}_{ij}| = |\bm{R}_i - \bm{R}_j|$ we can determine the velocity of $i^{th}$ sphere exactly from the surface tractions integral by combining Faxen's law\cite{Faxen_1922} cite with the Oseen tensor.
\begin{equation}
    \bm{V_i} =
    -\mu_0^t \bm{F}_i
    + \sum_{j\neq i}^N
...
\begin{eqnarray}
    ...
    \mm{\mu}_{ij}^{tt,RP} & = & \mu_0^t \left( \frac{3}{4} \left( \frac{a}{R} \right) \left( \mm{1} + \mm{\hat{R}}\mm{\hat{R}} \right) + \frac{1}{2} \left( \frac{a}{R} \right)^3 \left( \bm{1} -3  \mm{\hat{R}}\mm{\hat{R}} \right) \right) \quad \mathrm{for} \quad i \neq j.
...
First, by deonting expectations as $\left< \cdot \right>$, Ito integrals are martingales
\begin{equation}
...
For a more detailed overview the subject matter see \textcite[chapter 3.
]{Dhont_2008}.
These scalings are valid under the assumption of small persistence length $P = \frac{EI}{k_B T}$ which captures the ratio of elastic forces (proportional to rigidity $EI$) to Brownian forces (proportional to fluctuation energy $k_B T$).
